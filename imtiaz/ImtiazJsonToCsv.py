...
class ImtiazJsonToCsv(BaseJsonToCsv):
    ...
    def parse_menu_csv(self):
        ...
        logging.info(f"[{self.get_service_name()}] Starting CSV generation for {total_records} menu files")
        ...
        try:
            ...
            for product in products:
                try:
                    ...
                    # Use sub-section as parent category (we added this metadata in menu scraper)
                    sub_section_name = product.get('sub_section_name', 'N/A')
                    sub_section_id = str(product.get('sub_section_id', 'N/A'))
                    
                    # Additional product metadata
                    brand_name = product.get('brand_name', '')
                    availability = product.get('availability', 1)
                    
                    # Skip unavailable products if needed (optional)
                    # if availability == 0:
                    #     logging.debug(f"[{self.get_service_name()}] Skipping unavailable product: {menu_name}")
                    #     skipped_items += 1
                    #     continue
